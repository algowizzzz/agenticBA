        ...
        def _run_financial_sql_wrapper(query: str) -> Dict[str, str]:
            ...
            try:
                ...
                if not result or "error" in result.lower():
                    logger.warning(
                        f"[Financial Tool] Chain returned potentially empty or error result: {result}"
                    )
                ...
                logger.error(
                    f"[Financial Tool] Error during SQL chain execution: {e}",
                    exc_info=True,
                )
            ...
            end_time = time.time()
            logger.info(
                f"[Financial Tool] Query execution time: {end_time - start_time:.2f}s"
            )
