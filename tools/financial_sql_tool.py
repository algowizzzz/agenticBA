# Wrapper for financial SQL queries
import logging
import time
import sqlite3
...
def run_financial_sql(query: str, llm: BaseChatModel, db_path: str) -> Dict[str, Any]:
    """
    Generates and executes SQL via LLM for the financial DB.
    Returns a dictionary {"sql_query": ..., "sql_result": ..., "error": ...}.
    Includes granular error handling for generation and execution.
    """
    logger.info(f"[Financial Tool] Initializing for query: {query[:100]}... DB: {db_path}")
    ...
    try:
        ...
        try:
            ...
            logger.info(f"[Financial Tool] DB Connection OK. Tables: {tables}")
        except Exception as db_conn_err:
            logger.error(f"[Financial Tool] Error connecting to DB: {db_conn_err}", exc_info=True)
            error = f"Error: Could not connect to Financial DB at {db_path}. Details: {type(db_conn_err).__name__}"
        ...
        try:
            ...
            logger.error(f"[Financial Tool] Error during SQL generation: {llm_err}", exc_info=True)
        ...
        try:
            ...
            logger.info(f"[Financial Tool] SQL execution successful. Result: {str(sql_result)[:200]}...")
        except sqlite3.Error as db_err: # Catch specific DB errors
            logger.error(f"[Financial Tool] Database Error executing SQL: {db_err}. SQL: {generated_sql}")
            ...
            logger.error(f"[Financial Tool] Unexpected error executing SQL: {exec_err}. SQL: {generated_sql}", exc_info=True)
        ...
        logger.error(f"[Financial Tool] Unexpected error in tool wrapper: {e}", exc_info=True)
        error = f"Error: Unexpected internal tool error: {type(e).__name__}"

    end_time = time.time()
    logger.info(f"[Financial Tool] Query processing time: {end_time - start_time:.2f}s")
