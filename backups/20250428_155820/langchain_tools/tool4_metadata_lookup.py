...
def llm_metadata_lookup(query_term: str) -> Dict[str, Any]:
    ...
    if not api_key:
        logger.error("Metadata Tool: ANTHROPIC_API_KEY not set in environment variables")
        default_error_return["error"] = "API key not configured. Please set the ANTHROPIC_API_KEY environment variable."
        return default_error_return

    # Log API key information (safely)
    logger.info(f"Metadata Tool: Found API key in environment (length: {len(api_key)}, starts with: {api_key[:5]}..., ends with: ...{api_key[-5:]})")
    ...
    # --- Prepare LLM --- 
    try:
        logger.info("Metadata Tool: Initializing ChatAnthropic with model: claude-3-5-sonnet-20240620")
        llm = ChatAnthropic(
        ...
        logger.info("Metadata Tool: ChatAnthropic initialized successfully")
    except Exception as e:
        error_type = type(e).__name__
        error_msg = str(e)
        logger.error(f"Metadata Tool: Failed to initialize LLM: {error_type} - {error_msg}", exc_info=True)
        ...
        if "authentication" in error_msg.lower() or "401" in error_msg or "api key" in error_msg.lower():
            ...
            logger.critical(f"Metadata Tool: API KEY AUTHENTICATION ERROR: {detailed_error}")
            default_error_return["error"] = f"Authentication error (AUTH-401): {error_msg}"
        else:
            default_error_return["error"] = f"LLM Initialization Error: {error_type} - {error_msg}"
    ...
    def _call_llm_with_metadata(documents_part: Dict[str, Any], part_num: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Metadata Tool: {error_type} during LLM call (Part {part_num}): {error_msg}", exc_info=True)
            
            # Check for authentication errors specifically
            if "authentication" in error_msg.lower() or "401" in error_msg or "api key" in error_msg.lower():
                logger.critical(f"Metadata Tool: API AUTHENTICATION ERROR during call (Part {part_num}): {error_msg}")
    ...
    if not result1 and not result2:
        error_msg = "Both LLM calls failed to produce valid results."
        if "authentication" in final_results.get("error", "").lower() or "api key" in final_results.get("error", "").lower():
            error_msg = "Authentication error with Anthropic API. Please check your API key."
...
def get_metadata_lookup_tool(api_key: Optional[str] = None) -> Callable:
    ...
    if api_key:
        ...
        logger.info(f"Metadata Tool: Set API key from parameter (length: {len(api_key)})")
    else:
        existing_key = os.getenv("ANTHROPIC_API_KEY")
        if existing_key:
            logger.info(f"Metadata Tool: Using existing API key from environment (length: {len(existing_key)})")
        else:
            logger.warning("Metadata Tool: No API key provided or found in environment")
