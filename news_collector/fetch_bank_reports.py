...
"""
Bank Report Fetcher
Retrieves SEC reports for specified bank CIK numbers and stores them locally.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("bank_report_fetcher.log"),
        logging.StreamHandler(sys.stdout)
    ]
)
logger = logging.getLogger("bank_report_fetcher")
...
# Known CIK codes (can be expanded)
# Note: SEC requires CIK with leading zeros (total 10 digits)
BANK_CIKS = {
    "0000009622": {"name": "BANK OF MONTREAL", "ticker": "BMO", "country": "Canada"}
...
def add_banks_to_database():
    """Add known banks to the database."""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    for cik, bank_info in BANK_CIKS.items():
        cursor.execute(
    ...
    logger.info(f"Added {len(BANK_CIKS)} banks to database.")

def download_cik_lookup():
    """
    Download the company tickers list from SEC and create a CIK lookup file.
    This provides an offline reference of CIK numbers.
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Return the BMO CIK if found
            bmo_info = df[df['ticker'] == 'BMO']
            if not bmo_info.empty:
                cik = bmo_info.iloc[0]['cik']
                logger.info(f"Found BMO in SEC database: CIK {cik}")
                # Update our bank CIKs dictionary
                BANK_CIKS[cik] = {"name": "BANK OF MONTREAL", "ticker": "BMO", "country": "Canada"}
...
def download_report_content(report):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if table:
                # Look for main document (usually 10-K, 10-Q, 8-K document itself)
                ...
                if main_doc_url:
                    ...
                    if doc_response.status_code == 200:
                        ...
                        metadata = {
                            "cik": report['cik'],
                            "company_name": BANK_CIKS.get(report['cik'], {}).get("name", "Unknown"),
                            "ticker": BANK_CIKS.get(report['cik'], {}).get("ticker", "Unknown"),
...
def download_all_reports():
    ...
    cursor.execute("SELECT * FROM reports WHERE downloaded = 0")
    reports = cursor.fetchall()
    
    conn.close()
    
    if not reports:
        logger.info("No new reports to download.")
        return
    
    logger.info(f"Found {len(reports)} reports to download.")
...
def main():
    """Main function to fetch and store bank reports."""
    logger.info("Starting Bank Report Fetcher...")
    
    # Download CIK lookup reference (also updates BANK_CIKS if found)
    cik_count = download_cik_lookup()
    logger.info(f"Downloaded reference with {cik_count} company CIKs")
    
    # Initialize database and add banks
    initialize_database()
    add_banks_to_database()
    
    # Process each bank
    for cik in BANK_CIKS.keys():
    ...
    logger.info("Bank Report Fetcher completed successfully.")
